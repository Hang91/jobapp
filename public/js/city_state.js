...
city_states['China'] = 'Beijing|Anhui|Chongqing|Fujian|Gansu|Guangdong|Guangxi|Guizhou|Hainan|Hebei|Heilongjiang|Henan|HongKong|Hubei|Hunan|Jiangsu|Jiangxi|Jilin|Liaoning|Macau|Nei Mongol|Ningxia|Qinghai|Shaanxi|Shandong|Shanghai|Shanxi|Sichuan|Taiwan|Tianjin|Xinjiang|Xizang (Tibet)|Yunnan|Zhejiang';
...
function set_country(oRegionSel, oCountrySel, oCity_StateSel, addOrSearch)
{
	var countryArr;
	oCountrySel.length = 0;
	oCity_StateSel.length = 1;
	...
	if (countries[region])
	{
		...
		if(addOrSearch){
			oCountrySel.options[0] = new Option('-----ALL-----','');
		}
		else{
			oCountrySel.options[0] = new Option('PLEASE SELECT A COUNTRY','');
		}
		...
		if(addOrSearch){
			oCity_StateSel.options[0] = new Option('-----ALL-----','');
		}
		else{
			oCity_StateSel.options[0] = new Option('PLEASE SELECT A STATE OR PROVINCE','');
		}
	}
	else{
...
function set_city_state(oCountrySel, oCity_StateSel, addOrSearch)
{
	var city_stateArr;
